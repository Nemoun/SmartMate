...
> 当前版本已经完成基础任务 CRUD、用户自定义类别、Finish-to-Start依赖、原子新建依赖、有向依赖图、依赖感知排序、推荐理由、实时搜索、组合筛选、首版任务统计仪表盘和三花猫桌宠。主窗口提供任务、依赖图、统计、设置四个入口，以及“现在做”任务槽、逾期提醒、卡片详情和可持久化外观/桌宠设置。任务、类别、依赖、统计事件以及专注会话通过 SQLite Schema v5 保留；专注 Model Service、Contracts 与 ViewModel 已实现自由正计时、秒级投影、最近记录、检查点、恢复和任务状态保护。专注页面与导航尚未实现，当前没有可见入口。
...
- 专注计时（Model/ViewModel 已实现）：Schema v5 和 `FocusService` 原子保存自由正计时会话、暂停时间段、检查点和任务快照，并在活动专注期间阻止任务完成或取消；`FocusContract` 与 `FocusViewModel` 已提供开始候选、秒级计时、强类型命令、保存警告和最近 50 条记录，页面与导航仍待下一阶段。详细方案见[专注功能设计](docs/focus-design.md)。
...
这样可以让任务列表、依赖图、桌宠和统计页面以不同方式展示同一份业务状态，同时保持算法和数据库代码可独立测试。依赖图继续遵循 Model 计算拓扑语义、ViewModel 计算像素布局和箭头几何、View 只绘制并转发稳定 ID；桌宠复用焦点与任务命令 Contracts，只为开关和位置建立独立设置切片；统计页面同样只消费 Contract 已聚合的数据，Qt Charts 不进入 Model 或 ViewModel。专注功能已经建立 Repository、Service 与 ViewModel Contract 链路，后续页面仍沿同一依赖方向接入。详细约束见[架构说明](docs/architecture.md)、[三花猫桌宠设计](docs/desktop-pet-design.md)、[数据统计设计](docs/statistics-design.md)、[专注功能设计](docs/focus-design.md)与[仓库开发规则](AGENTS.md)。
