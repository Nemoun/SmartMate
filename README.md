...
> 当前版本已经完成基础任务 CRUD、Finish-to-Start依赖、依赖感知排序、推荐理由、实时搜索和优先级筛选。任务与依赖通过 SQLite 在重启后保留；桌宠、专注计时和统计仍按后续阶段逐步实现。
...
- 任务依赖（已实现）：为待办任务编辑一个或多个前置任务，拒绝自依赖、重复依赖和循环依赖，并显示直接阻塞原因。
