...
> 当前版本已经完成基础任务 CRUD、用户自定义类别、Finish-to-Start依赖、原子新建依赖、有向依赖图、依赖感知排序、推荐理由、实时搜索、组合筛选、首版任务统计仪表盘和三花猫桌宠。主窗口提供任务、依赖图、统计、设置四个入口，以及“现在做”任务槽、逾期提醒、卡片详情和可持久化外观/桌宠设置。任务、类别、依赖、统计事件以及专注会话的 Schema v5 持久化底座通过 SQLite 保留；桌宠开关和悬浮位置通过 QSettings 保留。专注 Service 与页面尚未实现，当前没有可见的专注入口。
...
- 专注计时（持久化底座已实现）：Schema v5 已能原子保存自由正计时会话、暂停时间段、检查点和任务快照；`FocusService`、任务状态保护、页面与导航仍待后续阶段实现。详细方案见[专注功能设计](docs/focus-design.md)。
