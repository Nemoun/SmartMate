...
当前进度：阶段 1、2、3 已完成。`SmartMateWidgets` 已具备主窗口、导航、主题、外观设置和任务主流程；任务页通过独立的列表、焦点、详情和编辑器 Contract 接入同一 Model Service。类别管理、类别筛选、新建前置集合、依赖编辑与依赖图仍待后续阶段迁移，依赖图当前保留明确的占位页。
