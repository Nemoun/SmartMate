...
专注功能第一阶段的持久化底座已经实现，完整事实、Schema v5 原子语义和后续阶段见[专注功能设计](focus-design.md)。当前仅存在 Model 领域类型、Repository 端口与 SQLite Adapter；`FocusService`、ViewModel Contracts、Widgets 页面和组合根注入尚未实现。
...
- 专注持久化阶段由普通 C++ `FocusSession` 与 `FocusInterval` 保存会话和连续 Running 时间段；数据库不保存重复总专注时长，后续累计、有效时长和任务状态保护必须由 Model Service 负责。
...
Repository 在同一 SQLite 事务内写入会话与时间段，并重新确认任务状态和期望会话状态。活动会话、时间段和最近完成记录都以普通领域类型返回，Qt SQL 不得越过 Persistence 边界。`ITaskActivityRepository` 可按任务查询指定 UTC 时刻前最近的 Start 事件，用于建立可空历史关联。

本阶段没有改变 `TaskService` 的状态事务，也没有向 `AppBootstrapper` 或 `MainWindowDependencies` 注入专注对象。后续必须先由 `FocusService` 建立进行中资格、有效时间段、检查点和任务完成/取消保护，再按 `FocusPage → Focus Contract → FocusViewModel → FocusService` 接入页面。完整约束见[专注功能设计](focus-design.md)。
