...
任务 Widgets 页面由 `TaskListContract`、`TaskFocusContract`、`TaskDetailsContract` 和 `TaskEditorContract` 四个窄端口组成。焦点面板读取焦点投影，但状态命令仍转发给任务列表命令端口；详情对话框只维护详情选择，编辑对话框由 `sessionActive` 和草稿通知驱动。日期与预计用时只能通过类型化控件提交，确认对话框只能触发一次单项或原子批量命令。
