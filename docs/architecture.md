...
`TaskEditorViewModel`在新建模式下同时维护任务字段和前置任务选择草稿。两者以一个`TaskCreationRequest`提交给Service，不能由两个ViewModel分步保存。编辑已有任务时，它只能通过`TaskService::findEditableTask`建立草稿；归档任务由Model拒绝，ViewModel仅映射错误。`TaskGraphViewModel`只把Model提供的结构化图快照转换为节点像素坐标、贝塞尔控制点和箭头几何；拓扑层级与可见节点闭包仍由Model计算。
