...
Schema v5 已保存原始 `FocusSession` 与连续 Running 时间段，`FocusService` 也已负责可靠检查点、暂停扣除和累计时长；专注页面与专注统计尚未实现。后续 `StatisticsService` 读取完成会话及其原始时间段，计算今日/本周专注时长和每日趋势；跨本地午夜的会话应按显式时区的自然日切分。
