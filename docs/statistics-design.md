...
Schema v5 已保存原始 `FocusSession` 与连续 Running 时间段，但 `FocusService`、专注页面和专注统计尚未实现。后续 Repository 返回完成会话及其原始时间段，由 `StatisticsService` 扣除暂停区间并计算今日/本周专注时长和每日趋势；跨本地午夜的会话应按显式时区的自然日切分。
