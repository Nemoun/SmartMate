    ...
    , m_editDependencies(new QPushButton(tr("编辑前置任务"), this))
{
    ...
    layout->addWidget(m_title); layout->addWidget(m_summary);
    layout->addWidget(m_description, 1); layout->addWidget(m_schedule);
...
void TaskDetailsDialog::synchronize()
{
    ...
    m_schedule->setText(tr("截止：%1    预计：%2 分钟\n创建：%3    更新：%4")
        .arg(m_details.selectedDeadlineText().isEmpty() ? tr("未设置") : m_details.selectedDeadlineText())
